/*
 * CubeGroup is a class designed to make building and 
 * manipulating groups of cube colliders for SceneSpace 
 * easier.
 * 
 */
...
public class ColliderGroup : MonoBehaviour {
	...
	public void Start () {
		...
		prefabCube.transform.localScale = new Vector3(cube_x_len, cube_y_len, cube_z_len);		
		...
		for (int x_box_num=0; x_box_num < num_x_cubes; x_box_num++) {
			for (int y_box_num=0; y_box_num < num_y_cubes; y_box_num++) {
				for (int z_box_num=0; z_box_num < num_z_cubes; z_box_num++) {
					GameObject newCube = (GameObject) Instantiate(prefabCube, 
